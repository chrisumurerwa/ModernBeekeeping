...
// generate token
 const token = jwt.sign({ id: savedUser.id, role: savedUser.role, email: savedUser.email,name:savedUser.name }, process.env.JWT_SECRET_KEY!, { expiresIn: '1h' });
...
export const ForgotPassword = asyncWrapper(async (   
    ...
    const resetLink = `exp://192.168.1.116:8081/resetPassword`;
   const htmlContent = `
